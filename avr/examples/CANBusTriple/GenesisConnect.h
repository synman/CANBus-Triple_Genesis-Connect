...
Message GenesisConnect::process( Message msg ) {
  ...
  if (msg.frame_id == 0x10A) {
    ...
    if ((msg.frame_data[1] == 0x10 && (GenesisConnect::volume != msg.frame_data[2]) || 
      ...
      if ( GenesisConnect::radioStation != msg.frame_data[4] ) {
        stereoCount = 0;
      }
  ...
  if (msg.frame_id = 0x107 && GenesisConnect::audioSource == 0x02 && GenesisConnect::radioStation != lastStation) {

    if (msg.frame_data[3] == 0x01) {
      stereoCount++;
    } else {
      stereoCount = 0;
    }
    ...
    if (stereoCount >= 2) {      
      ...
      stereoCount = 0;
      lastStation = GenesisConnect::radioStation;
    }
...
//  if (msg.frame_id = 0x10B && GenesisConnect::audioSource == 0x10 && msg.frame_data[5] != GenesisConnect::xmBand) {                
//    delay(BT_SEND_DELAY);
//    activeSerial->write(0x7C);
//    activeSerial->write(0x7C);
